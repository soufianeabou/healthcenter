...
const MedicalRecord: React.FC<MedicalRecordProps> = ({ patient, visible, onClose }) => {
  ...
  const fetchConsultations = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:8080/api/consultations/patient/${patient.id}`);
      if (response.ok) {
  ...
  const handleAddConsultation = async (values: any) => {
    try {
      const response = await fetch(`http://localhost:8080/api/consultations/add`, {
        method: 'POST',
        headers: {
